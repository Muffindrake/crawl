...
{
public:
    UIMenu(Menu *menu) : m_menu(menu), vis_item_first(0), vis_item_last(-1), title_height(0), more_height(0)
...
protected:
    ...
    int get_scroll_y() const {
        return title_height - (item_info.size() ? item_info[vis_item_first].y : 0);
    };
...
#else
    return scroll_to_item(vis_item_first + (vis_item_last-vis_item_first+1)*delta);
#endif
}
...
{
    if ((index == vis_item_first && !force) || m_menu->items.size() == 0)
        return false;
    index = min(max(0, index), (int)m_menu->items.size()-1);
...
#else
    vis_item_first = index;
    vis_item_last = index + m_pagesize - 1;

    int extra = vis_item_last - m_menu->items.size() + 1;
    if (extra > 0)
    {
        vis_item_first = max(vis_item_first - extra, 0);
        vis_item_last = min(vis_item_first + m_pagesize, (int)m_menu->items.size()-1);
    }
    ASSERT(0 <= vis_item_first && vis_item_first < (int)m_menu->items.size());
    ASSERT(0 <= vis_item_last  && vis_item_last < (int)m_menu->items.size());
#endif
...
void UIMenu::do_layout(int mw, int mh)
{
    ...
    while (b < item_info.size())
    {
        ...
        {
            max_height = min(max_height, item_info[b].h + item_info[b].y - item_info[a].y);
    ...
    if (height > m_height)
        m_height -= more_height;

    m_nat_column_width = min(column_width, max_column_width);
}
#endif
...
UISizeReq UIMenu::_get_preferred_size(int dim, int prosp_width)
{
    ...
    if (!dim)
    {
        ...
        return {0, max({m_nat_column_width * m_num_columns,
                (int)m_font_entry->string_width(title),
                (int)m_font_entry->string_width(m_menu->more)})};
    }
    else
    {
...
int UIMenu::mouse_entry(const int x, const int y)
{
    for (int i = vis_item_first; i <= vis_item_last; ++i)
    {
        ...
        const int w = m_nat_column_width;
    ...
    for (int i = vis_item_first; i <= vis_item_last; ++i)
    {
        ...
        if (entry.heading)
        {
            ...
            if (i < (int)item_info.size()-1 && !item_info[i+1].heading)
                m_div_line_buf.add(entry.x, entry.y+scroll_y+5,
                        entry.x+m_num_columns*col_width, entry.y+scroll_y+5, header_div_colour);
            m_font_buf.add(split, entry.x, entry.y+scroll_y+8);
        }
        else
        {
    ...
    alive(false), last_selected(-1), m_kmc(kmc), m_filter(nullptr)
{
    ...
    set_flags(flags);
    set_more(formatted_string::parse_string("<blue>-more-</blue>"));
}
...
bool Menu::process_key(int keyin)
{
    if (m_filter)
    {
        ...
        if (key == -1)
            return true;
        bool validline = key != CK_ESCAPE;
        string linebuf = m_filter->get_text();
        bool ret = true;
        if (validline && linebuf[0])
            ret = filter_with_regex(linebuf.c_str());
        delete m_filter;
        m_filter = nullptr;
        update_title();
        return ret;
    ...
    case CONTROL('F'):
        if (flags & MF_ALLOW_FILTER)
        {
            ...
            if (validline && linebuf[0])
                ret = filter_with_regex(linebuf);
#else
            ASSERT(!m_filter);
            m_filter = new resumable_line_reader();
#endif
            update_title();
            return ret;
        }
...
{
    menu_ui->update_items(); // XXX check parameter?
    update_title();

    // XXX: need to update more?
#ifdef USE_TILE_WEB
    if (update_entries)
    {
