...
int show_description(const describe_info &inf, const tile_def *tile)
{
    ...
    if (!inf.title.empty())
    {
        ...
        title_hbox->align_items = UI_ALIGN_CENTER;
...
void describe_feature_wide(const coord_def& pos)
{
...

    {
        ...
        if (tile == TILE_FLOOR_NORMAL)
            tile = env.tile_flv(pos).floor;
        else if (tile == TILE_WALL_NORMAL)
            tile = env.tile_flv(pos).wall;
    ...
    if (crawl_state.game_is_hints())
    {
        feat_info f = { "", "", tile_def(TILEG_TODO, TEX_GUI)};
        f.title = "Hints.";
        f.body = trimmed_string(hints_describe_pos(pos.x, pos.y));
    ...
    for (const auto &feat : feats)
    {
        ...
        title_hbox->add_child(move(icon));
#endif
        auto title = make_shared<UIText>(feat.title);
        ...
        if (has_desc)
        {
            auto text = make_shared<UIText>(feat.body);
            if (&feat != &feats.back())
...
bool describe_item(item_def &item, function<void (string&)> fixup_desc)
{
    ...
    {
        auto tiles_stack = make_shared<UIStack>();
    ...
    auto title = make_shared<UIText>(name);
    title->set_margin_for_crt({0, 0, 0, 0});
    title->set_margin_for_sdl({0, 0, 0, 10});
    title_hbox->add_child(move(title));
    ...
    auto scroller = make_shared<UIScroller>();
    auto text = make_shared<UIText>(trimmed_string(fs_desc.to_colour_string()));
    text->wrap_text = true;
    scroller->set_child(text);
    ...
    if (!actions.empty())
    {
        ...
        auto footer = make_shared<UIText>();
...
#ifdef USE_TILE
    auto spell_icon = make_shared<UIImage>();
                      ...
                      const string &footer)
{
...
#ifdef USE_TILE_LOCAL
    auto dgn = make_shared<UIDungeon>();
    ...
    if (!inf.quote.empty())
    {
        auto more = make_shared<UIText>(_toggle_message);
        more->set_margin_for_crt({1, 0, 0, 0});
        more->set_margin_for_sdl({20, 0, 0, 0});
        vbox->add_child(move(more));
    }
